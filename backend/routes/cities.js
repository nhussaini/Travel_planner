...
module.exports = ({
  ...
  const roadGoatApiAuth = {
    auth: {
      username: process.env.ROADGOATUSER,
      password: process.env.ROADGOATSECRET,
    },
  };
  ...
  router.post("/getCityData", async (req, res) => {
    ...
    const googleCall = `https://maps.googleapis.com/maps/api/place/textsearch/json?query=attractions+${cityName}&key=${process.env.GOOGLEKEY}`;
